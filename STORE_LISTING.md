...
- English only. Summaries are AI output and can be wrong.
...
- `storage`: Used with `chrome.storage.local` to remember the user's chosen summary style and detail level on their device. Page text and summaries are not stored.
...
5. Try Key points / Headline and the detail level under "More options". Chrome-internal pages, the Web Store, and PDFs are intentionally unsupported and show a message.
